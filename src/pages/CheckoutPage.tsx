...
const checkoutSchema = z.object({
    ...
    lastName: z.string().min(2, "Last name must be at least 2 characters").regex(/^[a-zA-Z\s'-]+$/, "Last name can only contain letters"),
    email: z.string().email("Please enter a valid email address"),
    address: z.string().min(5, "Address must be at least 5 characters"),
    city: z.string().min(2, "City must be at least 2 characters"),
    zip: z.string().min(5, "ZIP code must be at least 5 characters").regex(/^[0-9-]+$/, "ZIP code can only contain numbers and dashes"),
});
...
export function CheckoutPage() {
    ...
    const onSubmit = (_data: CheckoutData) => {
                        ...
                        <div className="rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] p-6 md:p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div className="space-y-2">
                                    ...
                                    <Input id="first-name" placeholder="John" {...register("firstName")} className={inputClasses} />
                                    {errors.firstName && <p className="text-[var(--color-error)] text-xs font-sans">{errors.firstName.message}</p>}
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="last-name" className="text-sm text-[var(--color-text-muted)] font-sans">Last name</Label>
                                    <Input id="last-name" placeholder="Doe" {...register("lastName")} className={inputClasses} />
                                    {errors.lastName && <p className="text-[var(--color-error)] text-xs font-sans">{errors.lastName.message}</p>}
                                </div>
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="email" className="text-sm text-[var(--color-text-muted)] font-sans">Email</Label>
                                <Input id="email" type="email" placeholder="john@example.com" {...register("email")} className={inputClasses} />
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="address" placeholder="123 Main St" {...register("address")} className={inputClasses} />
                                {errors.address && <p className="text-[var(--color-error)] text-xs font-sans">{errors.address.message}</p>}
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                                <div className="space-y-2 sm:col-span-2">
                                    <Label htmlFor="city" className="text-sm text-[var(--color-text-muted)] font-sans">City</Label>
                                    <Input id="city" placeholder="Anytown" {...register("city")} className={inputClasses} />
                                    {errors.city && <p className="text-[var(--color-error)] text-xs font-sans">{errors.city.message}</p>}
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="zip" className="text-sm text-[var(--color-text-muted)] font-sans">ZIP code</Label>
                                    <Input id="zip" placeholder="12345" {...register("zip")} className={inputClasses} />
                                    {errors.zip && <p className="text-[var(--color-error)] text-xs font-sans">{errors.zip.message}</p>}
                                </div>
                            </div>
                        </div>
                    </div>
